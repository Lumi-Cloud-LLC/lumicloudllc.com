  ...
  const items: MotifItem[] = useMemo(() => {
    ...
    for (let i = 0; i < effectiveCount; i++) {
      ...
      if (lane === 0) {
        ...
        x = `right:${(2 + Math.random() * 9).toFixed(1)}%`
      } else {
        x = `left:${(34 + Math.random() * 30).toFixed(1)}%`
      }
      ...
      result.push({ fr, lane, x, oprand, depthValue, scale, driftAnim, motifType: i % MOTIF_SVGS.length })
    }
    return result
